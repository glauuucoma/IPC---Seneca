...
int importAppointments(const char* datafile, struct Appointment appoints[], int max) {
    ...
    if (fp != NULL) {
        ...
        for (i = 0; i < max && !feof(fp); i++) {
            fscanf(fp, "%d,%d,%d,%d,%d,%d", &appoints[i].patientNumber, &appoints[i].date.year, &appoints[i].date.month, &appoints[i].date.day, &appoints[i].time.hour, &appoints[i].time.min); // Acess values and print them out
            
            if (!feof(fp)) { // As long as it's not end of file add 1 to count
                count++;}
        }
